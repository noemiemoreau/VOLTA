...
def train(train_loader, model, criterion, optimizer, epoch, config, labeling_module):
    ...
    for i, (images, _, patch, slide_id, coordinates, mask, segmentation, extra_feat) in enumerate(train_loader):
        ...
        images_0 = Image.fromarray(np.moveaxis(images[0][0].numpy(), 0, -1))
        images_1 = Image.fromarray(np.moveaxis(images[1][0].numpy(), 0, -1))
        images_0.save(config['save_dir'] + '/image_0.png')
        images_1.save(config['save_dir'] + '/image_1.png')
